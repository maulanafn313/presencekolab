...
const pages = { rekap: qs('#page-rekap'), 'laporan-bulanan': qs('#page-laporan-bulanan'), members: qs('#page-members'), laporan: qs('#page-laporan'), 'admin-monthly': qs('#page-admin-monthly'), dashboard: qs('#page-dashboard'), settings: qs('#page-settings'), 'help-requests': qs('#tab-help-requests'), 'qa-panel': qs('#page-qa-panel') };
...
function openMobileSidebar() {
    const sidebarAdmin = qs('#sidebar-admin');
    const sidebarPegawai = qs('#sidebar-pegawai');
    const sidebar = sidebarAdmin || sidebarPegawai;
...
function showPage(name, pushState = true){ 
    ...
    if(name==='qa-panel' && typeof window.qaInitOnShow === 'function') window.qaInitOnShow();
}
...
async function renderManualHolidays(){
    ...
    renderPaginatedTable(tbodyId, list, (it) => {
        ...
        tr.innerHTML = `<td class="py-2 px-3">${it.date}</td><td class="py-2 px-3">${it.name}</td><td class="py-2 px-3 text-center"><button type="button" class="mh-del bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded" data-id="${it.id}">Hapus</button></td>`;
        return tr;
    }, {
        colSpan: 3,
...
document.addEventListener('click', (e) => {
    // Year pagination in monthly report list
    const pageBtn = e.target.closest('.page-btn');
    if (pageBtn) {
        const year = parseInt(pageBtn.getAttribute('data-year'));
        if (!isNaN(year)) {
            currentMonthlyPageYear = year;
            renderMonthly();
        }
    }

    // Open create monthly report form
    const createBtn = e.target.closest('.btn-create-month');
    if (createBtn) {
        const year = createBtn.getAttribute('data-year');
        const month = createBtn.getAttribute('data-month');
        
        if (qs('#monthly-report-month')) qs('#monthly-report-month').value = month;
        if (qs('#monthly-report-year')) qs('#monthly-report-year').value = year;
        
        // Setup initial rows
        const achBody = qs('#table-achievements-body');
        const obsBody = qs('#table-obstacles-body');
        if (achBody) achBody.innerHTML = '';
        if (obsBody) obsBody.innerHTML = '';
        if (typeof addAchievementRow === 'function') addAchievementRow();
        if (typeof addObstacleRow === 'function') addObstacleRow();
        
        const form = qs('#form-monthly-report');
        if (form) form.reset();
        
        const titleElement = qs('#monthly-report-title');
        if (titleElement && typeof monthName === 'function') {
            titleElement.textContent = `Laporan Bulanan - ${monthName(parseInt(month))} ${year}`;
        }
        
        if (typeof pageMonthlyList !== 'undefined' && typeof pageMonthlyForm !== 'undefined') {
            pageMonthlyList.classList.add('hidden');
    ...
    try {
        ...
        if (result.ok && result.data) {
            ...
            const formatTime = (t) => {
                ...
                let hour = parseInt(tStr);
                if(isNaN(hour)) return tStr;
                return (hour < 10 ? '0' : '') + hour + ':00';
            };

            qs('#min-checkin-hour').value = formatTime(settings.min_checkin_hour?.value || '04:00');
            qs('#max-ontime-hour').value = formatTime(settings.max_ontime_hour?.value || '08:00');
            qs('#min-checkout-hour').value = formatTime(settings.min_checkout_hour?.value || '17:00');
            if(qs('#default-checkout-time')) qs('#default-checkout-time').value = formatTime(settings.default_checkout_time?.value || '17:00');
